...
pdf("./figures/240_x2active_gene_expression.pdf", width = 6, height = 4)
for (i in 1:3) {
    print(i)
    ...
    gene_un_2active <- d_stat_trans[trans %in% c("Un->Active", "Un->Active->Un") & group == x_group, gene] %>% intersect(rownames(mat))
    gene_bi_2active <- d_stat_trans[trans %in% c("Bivalent->Active", "Bivalent->Active->Bivalent") & group == x_group, gene] %>% intersect(rownames(mat))

    length(gene_un_2active)
    length(gene_bi_2active)

    x <- so_sub@meta.data$ct

    # (exp(mat) - 1) / (exp(mat[, "HSCs & MPPs"]) - 1) -> mat

    y <- mat[gene_un_2active, ]
    d_plot_un2a = melt(y)
    d_plot_un2a$cat = "un2a"

    y <- mat[gene_bi_2active, ]
    d_plot_bi2a = melt(y)
	...
	ggtitle(paste0(names(celltype_order_l)[i], " lineage with ", length(gene_un_2active), " un2a genes and ", length(gene_bi_2active), " bi2a genes"))
    print(p)
}
...
pdf("./figures/240_plot_x2active_fold_change > 2.pdf", width = 6, height = 4)
for (i in 1:3) {
    print(i)
...
pdf("./figures/240_scatter_plot_x2active_RNA_pseudotime.pdf", width = 6, height = 4)
...
for (i in 1:3) {
    ...
    gene_un_2active = d_activated_genes[cat == "un2a", gene] %>% intersect(rownames(mat)) %>% unique
    gene_bi_2active = d_activated_genes[cat == "bi2a", gene] %>% intersect(rownames(mat)) %>% unique
    # gene_all = d_activated_genes[cat == "all", gene] %>% intersect(rownames(mat)) %>% unique

    y_u2a = mat[gene_un_2active, ] %>% apply(2, mean)
    y_b2a = mat[gene_bi_2active, ] %>% apply(2, mean)
	...
	ct = d$ct,
	psudotime = psudotime_v,
	Un2Active = scale(y_u2a),
	Bi2Active = scale(y_b2a)
	# All = scale(y_all)
    ) 
    d_plot = d_plot[ct %in% x_celltype_order]
    d_plot$psudotime %<>% rank(ties = "random")
	...
	geom_point(alpha = 0.2, size = 0.1) +
	geom_smooth(method = "loess", se = F, span = 0.7) +
	scale_color_manual(values = c("Bi2Active.V1" = "#e18726", "Un2Active.V1" = "#1972b5")) +
	theme_classic() +
	xlab(paste0(names(celltype_order_l)[i], " pseudotime")) +
	theme(axis.text.x = element_text(angle = 45, hjust = 1))
    print(p)
...
pdf("./figures/240_gene_scRNA_pseudotime.pdf")
for (i in 1:3) {
    gene_v = gene_l[[i]]
