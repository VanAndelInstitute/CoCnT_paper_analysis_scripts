...
## read data
d_consensus = fread("./tmp/table_gene_chromatin_status_consensus_by_celltype_wide.tsv")[!is.na(HSPC)]

res = list(
    process_trajectory(d_consensus, ct_v_B, "B_consensus"),
    process_trajectory(d_consensus, ct_v_E, "E_consensus"),
    process_trajectory(d_consensus, ct_v_M, "M_consensus")
) %>% rbindlist


## count of shared genes between trajectories
res[, lineage := group %>% substr(1, 1)]

d_wide = dcast(res, gene ~ group, value.var = "trans", fill = "None")

write_tsv(d_wide, "./tmp/table_gene_chromatin_status_trajectory_consensus_remove_dup_wide.tsv")
write_tsv(res, "./tmp/table_gene_chromatin_status_trajectory_consensus_remove_dup_long.tsv")
...
d_summary_wide[order(-B_consensus)][1:30]

write_tsv(d_summary, "./tmp/table_gene_chromatin_status_trajectory_consensus_remove_dup_summary.tsv")
write_tsv(d_summary_wide, "./tmp/table_gene_chromatin_status_trajectory_consensus_remove_dup_summary_wide.tsv")

pdf("./figures/figure_chromatin_status_trajectory_consensus_barplot.pdf", width = 10, height = 8)
...
v_trans_selected = d_summary[N > 100, trans] %>% unique
...
d_plot = res[, .N, by = .(gene, trans, lineage)][, .(status = paste(sort(lineage), collapse = "+")), by = .(gene, trans)]

Cairo::CairoPDF("./figures/heatmap_shared_genes_between_lineages_in_trajectory_consensus.pdf", width = 12, height = 8)

row_order = c("B+M+E", "B+M", "B+E", "E+M", "B", "M", "E")
...
plot_shared_genes_heatmap <- function(d, trans_order, main_title) {
    d_plot <- dcast(d, trans ~ status, fill = 0)
    mat <- as.matrix(d_plot[, -1])
    rownames(mat) <- d_plot$trans
    # mat <- log10(mat + 1)
    mat <- mat[, c("B+E+M", "B+M", "B+E", "E+M", "B", "M", "E")]
    mat <- mat[trans_order, ]
    pheatmap(
	mat,
	cluster_rows = T,
